...
for indexL = 1:length(locationList)
    ...
    if(~isempty(portfolioValues))
        ...
        % Madagascar fix: Handle empty access codes (no access costs)
        if ~isempty(portfolioAccessCodes{indexSorted(1)})
            locationAccessCodes{indexL} = portfolioAccessCodes{indexSorted(1)}(1,1:end-2);
        else
            locationAccessCodes{indexL} = [];
        end
    else
        agent.trapped = 1;
...
% Madagascar fix: Handle empty access codes (no access costs)
if indexSorted(1) <= length(locationAccessCodes) && ~isempty(locationAccessCodes{indexSorted(1)})
    ...
    % No access costs to pay for Madagascar
    codesToPay = [];
    newCosts = 0;
end
